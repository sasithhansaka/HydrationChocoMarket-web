<?php 
    include 'conf.php';
    $sql = "SELECT * FROM feastables";
    $result = mysqli_query($conn, $sql);
    if(isset($_POST['filter'])){
        $sql = "SELECT * FROM feastables ORDER BY price DESC";
        $result = mysqli_query($conn, $sql);
     }
     if(isset($_POST['filterl'])){
        $sql = "SELECT * FROM feastables ORDER BY price ASC";
        $result = mysqli_query($conn, $sql);
     }
     if(isset($_POST['out'])){
        $sql = "SELECT * FROM feastables  where  quantity='0'";
        $result = mysqli_query($conn, $sql);
     }
     if(isset($_POST['in'])){
        $sql = "SELECT * FROM feastables where  quantity>'0'";
...
?>
   ...
   <nav class="navbar navbar-expand-lg " style="background-color: whitesmoke;height: 75px;border-color:rgb(46, 46, 46);border-bottom: 1px solid;">
    ...
    <div class="first">
    <div class="top">
    <form action="feastables.php" method="post">
    ...
    <div>
        ...
        <div class="imagea">
            <div class="imageb">
                ...
                <a class="ccc" href="feastblesstore.php?id=<?php echo $id ?>">BUY NOW</a>
            </div>         
        </div> 
            </div>
    <?php }} ?>
