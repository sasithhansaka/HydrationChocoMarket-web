<?php 
    ...
    if(isset($_POST['filter'])){
        $sql = "SELECT * FROM energy ORDER BY price DESC";
        $result = mysqli_query($conn, $sql);
     }
     if(isset($_POST['filterl'])){
        $sql = "SELECT * FROM energy ORDER BY price ASC";
        $result = mysqli_query($conn, $sql);
     }
     if(isset($_POST['out'])){
        $sql = "SELECT * FROM energy  where  quantity='0'";
        $result = mysqli_query($conn, $sql);
     }
     if(isset($_POST['in'])){
        $sql = "SELECT * FROM energy where  quantity>'0'";
    ...
    <div>
        ...
        <div class="imagea">
            <div class="imageb">
                ...
                <a class="ccc" href="energystore.php?id=<?php echo $id ?>">BUY NOW</a>
            </div>         
        </div> 
            </div>
    <?php }} ?>
